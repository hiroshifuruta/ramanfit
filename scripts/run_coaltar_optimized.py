...
print("="*70)
print("RAMAN SPECTRUM FITTING - OPTIMIZED")
...
# 3. D3 - SHIFTED HIGHER, NARROWER
gauss_d3 = GaussianModel(prefix='d3_')
pars.update(gauss_d3.make_params())
pars['d3_center'].set(value=1570, min=1540, max=1610)  # Allow wider range for fitting
pars['d3_sigma'].set(value=50/(2*np.sqrt(2*np.log(2))), min=15, max=80)  # Allow wider sigma range
pars['d3_amplitude'].set(value=10000, min=100)

# 4. G
lorentz_g = LorentzianModel(prefix='g_')
pars.update(lorentz_g.make_params())
pars['g_center'].set(value=1590, min=1560, max=1630)  # Allow wider range
pars['g_sigma'].set(value=79/2, min=20, max=80)
pars['g_amplitude'].set(value=50000, min=1000)

# 5. D' - Lorentzian (around 1520 cm-1, not 1622)
lorentz_dp = LorentzianModel(prefix='dp_')
pars.update(lorentz_dp.make_params())
pars['dp_center'].set(value=1520, min=1510, max=1530)  # Centered at 1520 cm-1 with tighter bounds
...
ax[0].axvspan(x[0], x[-1], alpha=0.1, color='green', label='Fit region (750-2000)')
ax[0].plot(x, y, 'k.', alpha=0.4, markersize=2, label='Fit data')
ax[0].set_ylabel('Intensity', fontsize=12)
ax[0].legend(loc='upper right', fontsize=10)
ax[0].grid(True, alpha=0.3)
ax[0].set_title('Full Spectrum - 750-2000 cm⁻¹ Region Selected', fontsize=12)
...
ax[1].set_title('Optimized Fit (750-2000 cm⁻¹) - D3 Shifted Higher & Narrower', fontsize=12)
...
print(f"  Max |residual|: {residual_max:.2f}")

plt.tight_layout()
plt.savefig('output/ramanfit_coaltar_optimized.png', dpi=150, bbox_inches='tight')
print("Plot saved: output/ramanfit_coaltar_optimized.png")
plt.show()
